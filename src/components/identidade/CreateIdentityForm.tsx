  ...
  return (
    <div className="rounded-[12px] p-4 flex flex-col gap-3"
      ...
      <input
        ...
        className="w-full bg-transparent border-b text-base italic outline-none placeholder:not-italic"
        style={{ color: "var(--offwhite)", borderColor: "var(--caramel)", placeholderColor: "var(--text-muted)" }}
        autoFocus
      ...
      <div className="flex gap-2">
        <button onClick={() => { setShow(false); setName(""); }} className="text-xs flex-1 py-2 rounded-[8px]"
          style={{ border: "0.5px solid rgba(255,255,255,0.1)", color: "var(--text-muted)" }}>
          Cancelar
        </button>
        <button onClick={handleSubmit} disabled={isPending} className="text-xs font-medium flex-1 py-2 rounded-[8px]"
