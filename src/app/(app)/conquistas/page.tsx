...
const CATEGORY_LABELS: Record<string, { label: string; color: string; bg: string }> = {
  streak:     { label: "Consistência",  color: "var(--caramel)",  bg: "var(--caramel-pale)"       },
  habitos:    { label: "Hábitos",       color: "var(--navy)",     bg: "rgba(27,43,75,0.08)"        },
  stacking:   { label: "Stacking",      color: "#0891b2",         bg: "rgba(8,145,178,0.08)"       },
  identidade: { label: "Identidade",    color: "#059669",         bg: "rgba(5,150,105,0.08)"       },
  xp:         { label: "XP & Nível",    color: "#d97706",         bg: "rgba(217,119,6,0.08)"       },
  emocoes:    { label: "Mente",         color: "#7c3aed",         bg: "rgba(124,58,237,0.08)"      },
};
...
async function ConquistasContent() {
  ...
  earned.forEach((b) => {
    const cat = BADGE_CATEGORIES[b.badge.slug] ?? "outros";
      ...
      <div className="flex flex-col gap-5 pb-4">
        ...
        {earned.length > 0 && (
          <div className="flex flex-col gap-4">
            ...
            {Object.entries(earnedByCategory).map(([cat, badges]) => {
              const catInfo = CATEGORY_LABELS[cat] ?? { label: cat, color: "var(--caramel)", bg: "var(--caramel-pale)" };
              return (
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {badges.map(({ badge }) => (
                      <div key={badge.id} className="rounded-[12px] p-3 flex flex-col items-center gap-2 text-center"
                        style={{ background: catInfo.bg, border: `0.5px solid ${catInfo.color}30` }}>
                        <div className="w-11 h-11 rounded-full flex items-center justify-center text-xl"
                          style={{ background: "var(--offwhite)", border: `1.5px solid ${catInfo.color}` }}>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
              {inProgress.map(({ badge, progress, total }) => {
                const cat = BADGE_CATEGORIES[badge.slug] ?? "outros";
                const catInfo = CATEGORY_LABELS[cat] ?? { label: cat, color: "var(--caramel)", bg: "var(--caramel-pale)" };
                return (
                  <div key={badge.id} className="rounded-[12px] p-4 flex items-center gap-3"
                    style={{ background: "var(--offwhite-2)", border: "0.5px solid var(--border-light)", opacity: 0.8 }}>
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                      ...
                      <div className="w-full rounded-full overflow-hidden" style={{ height: "3px", background: "var(--border-light)" }}>
                        <div className="h-full rounded-full transition-all"
                          style={{ width: `${total > 0 ? Math.min((progress / total) * 100, 100) : 0}%`, background: catInfo.color }} />
                      </div>
                    </div>
                  </div>
                );
